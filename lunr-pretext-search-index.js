...
},
{
  ...
  "body": " Analyzing Equations Numerically    To understand that numerical algorithms such as Euler's method allow the approximation of solutions to the initial value problems and that there are more efficient algorithms than Euler's method such as those algorithms that use the Runge-Kutta methods .  To understand that Taylor's Theorem is a very useful tool for studying differential equations.  To understand that error analysis of the rate of convergence is very important for any numerical algorithm.     As we've already seen, we can't always find a solution to a first-order initial value problem analytically. It could be the case that we cannot integrate a function in terms of elementary functions, for instance. (It's not you, it's the integral!) However, we may still need a reasonable approximation of the solution curve to make estimates. We will explore some methods in this section on numerically approximating solutions to these problems.  Our initial value problem provides us with one point on the desired solution curve, namely . Our goal is to produce a set of discrete points that can approximate points on the solution curve. For this purpose, we will insist that the -coordinates be equally spaced. That is, and the corresponding is an approximation of the solution at ; that is, . Once this list of points is obtained, it is usually plotted by connecting the points by straight-line segments; or, if you are using a sophisticated computer package, a curve fitting routine may be used to fit a smooth curve through the points.  The presence of a term may put us in mind of our previous work on defining definite integrals in terms of Riemann sums. As with that situation, our approximations improve as the step size shrinks, but the calculations become difficult, especially because we humans are prone to rounding off answers, leading to error accumulating as we perform more calculations. Using computers can fend this off for awhile, but even computers have a maximum word size and run into this issue after several steps. Also, different methods have different error estimates based on the size of .    Euler's method  We have already seen the usefulness of slope (or direction) fields in visualizing solution curves. If we consider the initial value problem , we recall that we would draw a short line segment of slope through the point . This line segment is part of the tangent line to the solution curve at .   Euler's method is one of the oldest and simplest numerical methods for obtaining approximate solutions to the initial value problem . Visually, the method says that we would start at our initial point and follow the tangent line of slope until we arrive at a point units to the right. This will bring us to the point . We recompute our slope as and follow a line segment of this slope from until we arrive at a point units further to the right. This will bring us to the point . We then adjust our slope to be and follow the line segment of that slope from to . We continue in this manner until we reach the point . (See .)   Euler's method      Notice that no derivatives of have to be evaluated, just itself.   Algorithm for Euler's Method  Given , , our goal is to find approximate values of on the interval .    Choose a small step size , with a positive integer.    For , compute     Plot the points and connect them by drawing straight lines. This results in a piecewise linear approximation to on the interval .       Approximate the solution of , on the interval .    We first note that .  Let's first try this with points. Then we have that . It is helpful to make a table. The values are all determined once is chosen.   Euler's method table                                               Thus .  A couple of comments:     The value of at the end of each row is used as the value of in the following row. The values of and come from the initial condition we're given.    The function is the slope function and depends on the differential equation we're given.    We can use MATLAB to produce a table:  % Define the differential equation y' = f(x, y) f = @(x, y) x - y; % Initial conditions x0 = 0; y0 = 1; xmax = 2; % Configuration num_steps = 4; % Number of steps to generate 4 points after the initial condition h = (xmax - x0)\/num_steps; % Initialize arrays to hold the values x = zeros(num_steps + 1, 1); y = zeros(num_steps + 1, 1); f_val = zeros(num_steps + 1, 1); % Set the initial point x(1) = x0; y(1) = y0; f_val(1) = f(x(1), y(1)); % Euler's Method loop for n = 1:num_steps % Calculate next y value: y_next = y_curr + h * f(x_curr, y_curr) y(n+1) = y(n) + h * f_val(n); % Advance x x(n+1) = x(n) + h; % Calculate f(x, y) at the new point (except for the last point, where it's optional) if n < num_steps + 1 f_val(n+1) = f(x(n+1), y(n+1)); end end % Create and display the table Step = (0:num_steps)'; EulerTable = table(Step, x, y, f_val, 'VariableNames', {'n', 'x_n', 'y_n', 'f(x_n,y_n)'}); disp(EulerTable);      The differential equation in this example is linear: . Using the method from , we get that . (Check this!) This gives the exact value of The absolute error in our numerically computed value of in the above table is If the calculations are redone with , that is with , the corresponding table is   Euler's method table                                                                           Thus .  The error in the approximate value of is now By cutting the step size in half the error has been cut approximately in half, from 0.146 to 0.070. shows the two approximate solutions together plotted with the exact solution.   Euler's method approximations to with        Use MATLAB to generate a table for the case from the example.     In-Class Work on Euler's Method   Use Euler's method to find an approximate value for to four decimal places, given that is the solution to the IVP using first , then . Feel free to use your calculator or MATLAB to do the calculations.    Unlike the previous problem, the initial value problem can be solved by us by hand. (The equation is separable.)  Solve the equation by hand and determine the values of , up to .   Then use Euler's method with and compare the values you get for , up to . Feel free to use your calculator or MATLAB to do the calculations.      Error and the Order of a Numerical Method  We demonstrated Euler's method above on a differential equation we knew how to solve; we would never use this method on such an equation. (Why approximate when we can get an exact answer, and then express that as a decimal?) This was done to convince you that such a method can get results.  We would use a numerical method in the case where we can't get a solution analytically. Because of this, we are duty bound to estimate the error in using the numerical method.  There are two different types of error that can occur when solving a differential equation numerically. The first is called round-off error and is due to rounding off or chopping the result after each step of the procedure, in order to store it on a computer with fixed word size. For example, if , and the computer has a word size of 4 decimal digits, then it would probably store as 0.3333, already introducing an error greater than 0.00003 into the calculation. Fortunately, modern computers can store a lot more digits than this so it is usually not a problem, unless the procedure requires a huge number of steps.  The type of error you really need to be concerned about is caused by the approximations made by whatever method you are using. This is called truncation error .  Suppose we're trying to solve the initial value problem , for . If the slope function can be differentiated enough times, could be approximated by the first terms of its Taylor series about : where the error is known by Taylor's theorem to be for some value of between and .  In particular, the truncation error in Euler's method is caused by dropping all but the first two terms in the Taylor series for . From , we see that the maximum error made by using the approximation is given by where is some value between and . If we know that for , then we can say that the truncation error in a single step (the local truncation error ) is bounded by Numerical analysts refer to this as an error of order 2 because the error is proportional to .  Since Euler's method requires steps to go from to , it may seem that the total error (the global truncation error ) is bounded by times the local truncation error. However, this is not the case. The error in each step is carried over to the next step, and so the global truncation error actually grows exponentially with . For a fixed interval length, the global truncation error is still essentially a constant times , but if the interval of integration is increased, the global truncation error increases exponentially. This should warn you that Euler's method over very long time intervals may produce large errors even if is small.  Numerical analysts refer to this as a method of first order . If we halve the step size, the error is reduced by a factor of 2. This means that Euler's method is not very efficient, and we will see that there are methods of higher order that are more efficient. It's an easy method to understand, but should only be used over a short interval.    In-Class Work on Euler's Method    Use Euler's method to find an approximate value for to four decimal places, given that is the solution to the initial value problem using first , then . Feel free to use your calculator or the MATLAB code provided in the example to do the calculations.      Unlike the previous problem, the initial-value problem can be solved by us by hand. (The equation is separable.)  Solve the equation by hand and determine the values , , up to .   Then use Euler's method with and compare the values you get for , , up to . Feel free to use your calculator or the MATLAB code posted above to do the calculations.      Fourth-Order Runge-Kutta Method  A lot of work was done in the first half of the twentieth century to develop methods that are far more accurate. Most computer algebra systems use what are called Runge-Kutta Methods, developed by two German mathematicians. One of the major motivations behind these methods was to find a way to avoid having to differentiate the slope function. The fourth-order Runge-Kutta Method, for example, uses four evaluations of the slope function on each -interval to produce an approximation that is accurate to the fourth order. That is, the error in each step is proportional to . If we halve the step size, the error is reduced by a factor of 32. If we reduce the step size by a factor of 10, the error is reduced by a factor of 100,000.  There are many different Runge-Kutta formulas for any given order, but the algorithm given below is the one that has gained acceptance because of its simplicity. Although it is tedious to compute by hand, it is easy to program for a calculator or computer.   Algorithm for the Fourth-Order Runge-Kutta Method  Given , , our goal is to find approximate values of on the interval .    Choose a small step size , with a positive integer.    For , compute     Plot the points and connect them by drawing straight lines. This results in a piecewise linear approximation to on the interval .     The fourth-order Runge-Kutta method is a very popular method for numerically solving differential equations. It is more accurate than Euler's method and is still relatively easy to implement. However, it does require four evaluations of the slope function for each step, which can be computationally expensive for complex functions.  Let's apply the fourth-order Runge-Kutta method to the same initial value problem we solved using Euler's method: , on the interval . We will use steps, which gives us a step size of .  % Define the differential equation function f = @(x, y) x - y; % Initial conditions and parameters x0 = 0; y0 = 1; xf = 2; steps = 4; h = (xf - x0) \/ steps; % Step size = 0.5 % Initialize variables x = x0; y = y0; % Display headers fprintf('%5s %10s\\n', 'x', 'y'); fprintf('%5.2f %10.6f\\n', x, y); % Runge-Kutta 4th Order Loop for i = 1:steps k1 = f(x, y); k2 = f(x + h\/2, y + h*k1\/2); k3 = f(x + h\/2, y + h*k2\/2); k4 = f(x + h, y + h*k3); y = y + (h\/6) * (k1 + 2*k2 + 2*k3 + k4); x = x + h; fprintf('%5.2f %10.6f\\n', x, y); end % Final result fprintf('\\nThe approximate value of y(2) is: %.6f\\n', y);    The fprintf statements in the code above are used to display the values of and at each step of the Runge-Kutta method. The final result is printed after the loop, showing the approximate value of . The command fprintf('%5.2f %10.6f\\n', x, y) prints the value of using a total width of 5 spaces, with 2 digits after the decimal point, followed by the value of using a total width of 10 spaces, with 6 digits.   Thus four steps of the fourth-order Runge-Kutta method give us , which is very close to the exact value of . The absolute error is approximately 0.00043, which is significantly smaller than the error we obtained using Euler's method with the same number of steps.  If is halved to 0.25 (i.e., ), the fourth-order Runge-Kutta method would yield an even more accurate approximation of , demonstrating the efficiency and accuracy of this method compared to Euler's method. Every time we halve the step size, the error is reduced by a factor of 32 per step, which is a significant improvement over the first-order accuracy of Euler's method. If we keep the overall interval size small, the fourth-order Runge-Kutta method can provide highly accurate results with relatively few steps. (See .)   Runge-Kutta Table                          Not all solutions to differential equations behave the same. A solution curve may behave one way with a given initial condition, and a completely different way with a slightly different initial condition. It is often to our benefit to figure out what initial conditions lead to what behavior. This is especially true when we are unable to find an analytical solution to the differential equation.  We previously plotted the slope field for the differential equation and we noted that no analytical solution could be found. The slope field and four solutions are plotted in .   Solutions of ,    It appears that there is a initial condition that separates solutions that blow up to infinity from those that pass through the parabola and then tend asymptotically to as . To be able to make accurate predictions about the behavior of the system, it is important to find this critical value of . We can use the fourth-order Runge-Kutta method to help us locate this critical value.  We could figure this out by using initial conditions of the form for some large (that is, assume the solution curve hits the parabola far out) and then integrating backwards to .  We can use the MATLAB code below to find the critical value of by integrating backwards from to .  % Define the constant K K = 4; % You can change this to any positive value % Define the differential equation: dy\/dx = y^2 - x odefun = @(x, y) y^2 - x; % Set the integration interval [start_x, end_x] for backward integration xspan = [K, 0]; % Define the initial condition: y(K) = sqrt(K) y0 = sqrt(K); % Solve the IVP using ode45 [x, y] = ode45(odefun, xspan, y0); % Extract and print the final value at x = 0 y_at_zero = y(end); fprintf('The value of y(0) is: %8.8f\\n', y_at_zero);  We consistently find that the critical value of is approximately 0.72901. So, for instance, we could check that if we take and integrate forward, the solutions should ultimately go in different directions. We can see this by using our previous Runge-Kutta code to integrate forward from with initial condition and see what we get for, say, the value of .  % Define our function f = @(x, y) y^2 - x; % Initial conditions and parameters x0 = 0; y0 = 0.72901; xf = 4; steps = 32; % reflects initial h = 0.125 h = (xf - x0) \/ steps; % Initialize variables x = x0; y = y0; for k = 1:6 % Runge-Kutta 4th Order Loop for i = 1:steps k1 = f(x, y); k2 = f(x + h\/2, y + h*k1\/2); k3 = f(x + h\/2, y + h*k2\/2); k4 = f(x + h, y + h*k3); y = y + (h\/6) * (k1 + 2*k2 + 2*k3 + k4); x = x + h; end; % Final result fprintf('\\nThe approximate value of y(4) is: %.8f\\n', y); % Set new number of steps, value of h, reset initial x and y steps = 2*steps; h = (xf - x0) \/ steps; x = x0; y = y0; end  We get the following results for with different step sizes:   Runge-Kutta Table    Number of steps      32  0.125  2.06498090    64  0.0625  1.91640489    128  0.03125  1.90724317    256  0.015625  1.90666497    512  0.0078125  1.90662862    1024  0.00390625  1.90662634     As we go, the value of gains one significant digit with each doubling of the number of steps. We can say that to five significant digits. As , we can conclude that the solution curve with initial condition tends asymptotically to as . However, try changing the value of y0 in the MATLAB code to or and see what happens to the value of . A small error in the initial condition can lead to a large error in the final result. This is an example of a sensitive dependence on initial conditions, which is a hallmark of chaotic systems.  What you need to realize is that if you are working on a machine that keeps only a small number of significant digits in its calculations, then if is very small, or a huge number of steps are needed, bad things can happen.      Euler's method by hand   In , use Euler's method to approximate to four decimals the solution of the given initial value problem. Carry out the calculations by hand , first using and then using . Compare the results.      , ,        , ,       Euler's method error   In , use Euler's method to approximate the indicated value to four decimals. First use and then use . Then find an analytical solution to each initial value problem and compare the results.      , ,        , ,       Euler vs. Runge-Kutta   In , use MATLAB to obtain a four-decimal approximation to the of the indicated value of the solution to the given initial value problem, first using Euler's method and then using the fourth-order Runge-Kutta method and then the Runge-Kutta method, each with . Compare the results.      , ,        , ,       "
},
{
...
{
  "id": "subsec-error-Euler-7",
  "level": "2",
  "url": "sec-num-analysis.html#subsec-error-Euler-7",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "local truncation error error of order 2 "
},
{
  "id": "subsec-error-Euler-8",
  "level": "2",
  "url": "sec-num-analysis.html#subsec-error-Euler-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "global truncation error "
},
{
  "id": "subsec-error-Euler-9",
  "level": "2",
  "url": "sec-num-analysis.html#subsec-error-Euler-9",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "first order "
...
{
  "id": "ws-Euler-method-2-1",
  "level": "2",
  "url": "sec-num-analysis.html#ws-Euler-method-2-1",
  "type": "Worksheet Exercise",
  "number": "1.6.3.1",
  "title": "",
  "body": " Use Euler's method to find an approximate value for to four decimal places, given that is the solution to the initial value problem using first , then . Feel free to use your calculator or the MATLAB code provided in the example to do the calculations.  "
},
{
  "id": "ws-Euler-method-3-1",
  "level": "2",
  "url": "sec-num-analysis.html#ws-Euler-method-3-1",
  "type": "Worksheet Exercise",
  "number": "1.6.3.2",
  "title": "",
  "body": " Unlike the previous problem, the initial-value problem can be solved by us by hand. (The equation is separable.)  Solve the equation by hand and determine the values , , up to .   Then use Euler's method with and compare the values you get for , , up to . Feel free to use your calculator or the MATLAB code posted above to do the calculations.  "
},
{
  "id": "subsec-fourth-order-RK-4",
  "level": "2",
  "url": "sec-num-analysis.html#subsec-fourth-order-RK-4",
  "type": "Note",
  "number": "1.6.8",
  "title": "Algorithm for the Fourth-Order Runge-Kutta Method.",
  "body": " Algorithm for the Fourth-Order Runge-Kutta Method  Given , , our goal is to find approximate values of on the interval .    Choose a small step size , with a positive integer.    For , compute     Plot the points and connect them by drawing straight lines. This results in a piecewise linear approximation to on the interval .    "
},
{
  "id": "tbl-RK",
  "level": "2",
  "url": "sec-num-analysis.html#tbl-RK",
  "type": "Table",
  "number": "1.6.9",
  "title": "Runge-Kutta Table",
  "body": " Runge-Kutta Table                        "
},
{
  "id": "subsec-fourth-order-RK-13",
  "level": "2",
  "url": "sec-num-analysis.html#subsec-fourth-order-RK-13",
  "type": "Example",
  "number": "1.6.10",
  "title": "",
  "body": " Not all solutions to differential equations behave the same. A solution curve may behave one way with a given initial condition, and a completely different way with a slightly different initial condition. It is often to our benefit to figure out what initial conditions lead to what behavior. This is especially true when we are unable to find an analytical solution to the differential equation.  We previously plotted the slope field for the differential equation and we noted that no analytical solution could be found. The slope field and four solutions are plotted in .   Solutions of ,    It appears that there is a initial condition that separates solutions that blow up to infinity from those that pass through the parabola and then tend asymptotically to as . To be able to make accurate predictions about the behavior of the system, it is important to find this critical value of . We can use the fourth-order Runge-Kutta method to help us locate this critical value.  We could figure this out by using initial conditions of the form for some large (that is, assume the solution curve hits the parabola far out) and then integrating backwards to .  We can use the MATLAB code below to find the critical value of by integrating backwards from to .  % Define the constant K K = 4; % You can change this to any positive value % Define the differential equation: dy\/dx = y^2 - x odefun = @(x, y) y^2 - x; % Set the integration interval [start_x, end_x] for backward integration xspan = [K, 0]; % Define the initial condition: y(K) = sqrt(K) y0 = sqrt(K); % Solve the IVP using ode45 [x, y] = ode45(odefun, xspan, y0); % Extract and print the final value at x = 0 y_at_zero = y(end); fprintf('The value of y(0) is: %8.8f\\n', y_at_zero);  We consistently find that the critical value of is approximately 0.72901. So, for instance, we could check that if we take and integrate forward, the solutions should ultimately go in different directions. We can see this by using our previous Runge-Kutta code to integrate forward from with initial condition and see what we get for, say, the value of .  % Define our function f = @(x, y) y^2 - x; % Initial conditions and parameters x0 = 0; y0 = 0.72901; xf = 4; steps = 32; % reflects initial h = 0.125 h = (xf - x0) \/ steps; % Initialize variables x = x0; y = y0; for k = 1:6 % Runge-Kutta 4th Order Loop for i = 1:steps k1 = f(x, y); k2 = f(x + h\/2, y + h*k1\/2); k3 = f(x + h\/2, y + h*k2\/2); k4 = f(x + h, y + h*k3); y = y + (h\/6) * (k1 + 2*k2 + 2*k3 + k4); x = x + h; end; % Final result fprintf('\\nThe approximate value of y(4) is: %.8f\\n', y); % Set new number of steps, value of h, reset initial x and y steps = 2*steps; h = (xf - x0) \/ steps; x = x0; y = y0; end  We get the following results for with different step sizes:   Runge-Kutta Table    Number of steps      32  0.125  2.06498090    64  0.0625  1.91640489    128  0.03125  1.90724317    256  0.015625  1.90666497    512  0.0078125  1.90662862    1024  0.00390625  1.90662634     As we go, the value of gains one significant digit with each doubling of the number of steps. We can say that to five significant digits. As , we can conclude that the solution curve with initial condition tends asymptotically to as . However, try changing the value of y0 in the MATLAB code to or and see what happens to the value of . A small error in the initial condition can lead to a large error in the final result. This is an example of a sensitive dependence on initial conditions, which is a hallmark of chaotic systems.  What you need to realize is that if you are working on a machine that keeps only a small number of significant digits in its calculations, then if is very small, or a huge number of steps are needed, bad things can happen.  "
},
{
  "id": "exercises-num-method-1-3",
  "level": "2",
  "url": "sec-num-analysis.html#exercises-num-method-1-3",
  "type": "Exercise",
  "number": "1.6.5.1",
  "title": "",
  "body": "   , ,    "
...
{
  "id": "exercises-num-method-1-4",
  "level": "2",
  "url": "sec-num-analysis.html#exercises-num-method-1-4",
  "type": "Exercise",
  "number": "1.6.5.2",
  "title": "",
  "body": "   , ,    "
...
{
  "id": "exercises-num-method-2-3",
  "level": "2",
  "url": "sec-num-analysis.html#exercises-num-method-2-3",
  "type": "Exercise",
  "number": "1.6.5.3",
  "title": "",
  "body": "   , ,    "
...
{
  "id": "exercises-num-method-2-4",
  "level": "2",
  "url": "sec-num-analysis.html#exercises-num-method-2-4",
  "type": "Exercise",
  "number": "1.6.5.4",
  "title": "",
  "body": "   , ,    "
...
{
  "id": "exercises-num-method-3-3",
  "level": "2",
  "url": "sec-num-analysis.html#exercises-num-method-3-3",
  "type": "Exercise",
  "number": "1.6.5.5",
  "title": "",
  "body": "   , ,    "
...
{
  "id": "exercises-num-method-3-4",
  "level": "2",
  "url": "sec-num-analysis.html#exercises-num-method-3-4",
  "type": "Exercise",
  "number": "1.6.5.6",
  "title": "",
  "body": "   , ,    "
